...
re1max=max(re1);
re1min=min(re1);
['Epsilon 1 max= ' num2str(re1max) ' and Epsilon 1 min= ' num2str(re1min)]

%Min-Max for e2
re2=p./(1-e2*cos(theta));
re2max=max(re2);
re2min=min(re2);
['Epsilon 2 max= ' num2str(re2max) ' and Epsilon 2 min= ' num2str(re2min)]
...
re3max=max(re3);
re3min=min(re3);
['Epsilon 3 max= ' num2str(re3max) ' and Epsilon 3 min= ' num2str(re3min)]
